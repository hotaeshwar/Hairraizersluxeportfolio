    ...
    <AnimatePresence>
      <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 sm:p-6 md:p-8">
        ...
        <motion.div
          ...
          className="relative w-full max-w-3xl rounded-3xl glass-panel gold-gradient-border shadow-[0_20px_50px_rgba(0,0,0,0.1),_0_0_35px_rgba(212,175,55,0.2)] overflow-hidden z-10 flex flex-col md:flex-row max-h-[90vh] md:max-h-[85vh] bg-white/95"
        >
          {/* Close Icon Button */}
          <button
            onClick={onClose}
            className="absolute top-4 right-4 z-20 p-1.5 rounded-full border border-gold-primary/20 text-[#1a1a1a]/70 hover:text-gold-primary hover:border-gold-primary/60 transition-all duration-300 bg-neutral-100/70 focus:outline-none"
          ...
          <div className="w-full md:w-7/12 p-6 md:p-8 flex flex-col justify-between overflow-y-auto max-h-[50vh] md:max-h-[85vh] scrollbar-thin">
            <div>
              ...
              <h3 className="font-serif text-3xl font-extrabold text-[#1a1a1a] tracking-wide mb-4">
              ...
              <p className="font-sans text-xs sm:text-sm text-[#1a1a1a]/70 font-light leading-relaxed mb-6">
                {service.description}
              </p>
                ...
                {service.features &&
                  service.features.map((feature, i) => (
                    <div key={i} className="flex items-start gap-3">
                      ...
                      <span className="font-sans text-xs sm:text-sm text-[#1a1a1a]/85 font-light leading-relaxed">
                        {feature}
                      </span>
                    </div>
                  ))}
            ...
            <div className="flex flex-col sm:flex-row gap-4 pt-4 border-t border-gold-primary/10 mt-auto">
              <button
                onClick={onBook}
                className="flex-1 flex items-center justify-center gap-2 py-3.5 px-6 rounded-xl border border-gold-primary bg-gold-primary text-black text-xs font-sans tracking-[0.2em] font-bold transition-all duration-300 hover:shadow-[0_0_20px_rgba(212,175,55,0.4)] hover:scale-[1.01] cursor-pointer"
              >
                BOOK THIS RITUAL
              </button>
              <button
                onClick={onClose}
                className="sm:w-32 py-3.5 px-6 rounded-xl border border-neutral-200 text-neutral-600 text-xs font-sans tracking-[0.2em] hover:text-black hover:border-neutral-400 transition-all duration-300 cursor-pointer text-center bg-transparent"
              >
                CLOSE
              </button>
            </div>
